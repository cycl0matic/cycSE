...
int read_sizes(int *linecounter, int *maxlength, char* docfile){
  ...
  int currentlength;
  while(-1 != (currentlength = getline(&line, &falsebuffer, file))){
  ...
  if(*linecounter == 0 || *maxlength < 3) {
    cout<<"Document doesn't meet the requirements"<<endl;
    return -1;
  }
  return 1;
}
...
int read_input(Mymap* mymap,Trienode *trie, char* docfile){
  ...
  size_t falsebuffer= 0;
  int currlength ;
  char* temp = (char*)malloc(mymap ->getbuffersize()*size(char));
